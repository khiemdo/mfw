...
nanopb_version = "nanopb-0.3.6-highwind"
...
class Field:
    ...
    def get_initializer(self, null_init, set_init, inner_init_only = False):
        ...
        if self.pbtype == 'MESSAGE':
            if null_init:
                ...
                if set_init:
                    inner_init = '%s_init_set' % self.ctype
                else:
    ...
    def default_decl(self, declaration_only = False):
        ...
        ctype = self.ctype
        default = self.get_initializer(False, False, True)
        array_decl = ''
...
class OneOf(Field):
    ...
    def get_initializer(self, null_init, set_init):
        return '0, {' + self.fields[0].get_initializer(null_init, set_init) + '}'
...
class Message:
    ...
    def get_initializer(self, null_init, set_init):
        ...
        for field in self.ordered_fields:
            parts.append(field.get_initializer(null_init, set_init))
        return '{' + ', '.join(parts) + '}'
    ...
    def generate_header(self, includes, headername, options):
        ...
        if self.messages:
            ...
            for msg in self.messages:
                ...
                yield '#define %-40s %s\n' % (identifier, msg.get_initializer(False, False))
            for msg in self.messages:
                identifier = '%s_init_set' % msg.name
                yield '#define %-40s %s\n' % (identifier, msg.get_initializer(False, True))
            for msg in self.messages:
                identifier = '%s_init_zero' % msg.name
                yield '#define %-40s %s\n' % (identifier, msg.get_initializer(True, False))
            yield '\n'
