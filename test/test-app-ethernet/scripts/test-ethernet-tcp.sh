...
DATA_PORT_TCP_LOG_FILE=log_port_tcp_data.txt
TARGET_RESPONSE=
...
function send_data
{
    ...
    nc.traditional -l -v -v -n -u -p $DEST_PORT_DATA 1> /dev/null 2>1 & # $DATA_PORT_RX_LOG_FILE &

    dd if=/dev/zero bs=$TX_DATA_BLOCK_SIZE count=$TX_DATA_REPETITIONS | nc.traditional -n -w 2 -v -v $DEST_IP $DEST_PORT_DATA_TCP > $DATA_PORT_TCP_LOG_FILE 2>&1
	if [ $? -ne 0 ]; then
		echo -e "${red}nc failed with code $?.${clean}"
		exit 1;
	fi
...
echo -e "Destination Port Data TCP :\t$DEST_PORT_DATA_TCP"
