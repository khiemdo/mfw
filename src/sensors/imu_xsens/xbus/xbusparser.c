 ...
 */
struct XbusParser* XbusParser_create(XbusParserCallback const* callback)
{
 ...
 */
struct XbusParser* XbusParser_init(void* parserMem, XbusParserCallback const* callback)
{
...
static void parseOutputConfig(struct XbusParser* parser, uint8_t const* rawData)
{
	...
	OutputConfiguration* conf = parser->callbacks.allocateBuffer(fields * sizeof(OutputConfiguration));
	if (conf)
	{
...
{
	uint8_t * const rawData = parser->currentMessage.data;
