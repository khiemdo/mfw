 ...
 */
static uint16_t messageLength(XbusMessage const* message)
{
	switch (message->mid)
	{
 ...
 */
static void formatOutputConfig(uint8_t* raw, XbusMessage const* message)
{
	OutputConfiguration* conf = message->data;
	size_t i;
 ...
 * raw bytes.
 */
static void formatPayload(uint8_t* raw, XbusMessage const* message)
{
	size_t i;
	switch (message->mid)
	{
 ...
 * a motion tracker.
 */
size_t XbusMessage_format(uint8_t* raw, XbusMessage const* message, enum XbusLowLevelFormat format)
{
	uint8_t* dptr = raw;
	switch (format)
	{
 ...
 */
bool XbusMessage_getDataItem(void* item, enum XsDataIdentifier id, XbusMessage const* message)
{
